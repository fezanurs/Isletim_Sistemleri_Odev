...
def run_sjf_preemptive(processes):
    ...
    while completed < n:
        ...
        current_proc = min(active_pool, key=lambda x: x['remaining'])
...
for f_name in files:
    print(f"Processing {f_name} for Preemptive SJF...")
    data = read_file(f_name)
    if not data: continue

    wt, tat, cs, total_time, logs = run_sjf_preemptive(data)

    avg_wt = sum(wt)/len(wt)
    avg_tat = sum(tat)/len(tat)
    efficiency = (sum(p['burst'] for p in data) / total_time) * 100
    
    out_name = f"Preemptive_SJF_Result_{f_name}.txt"
