...
def run_sjf_np(processes):
    ...
    while completed_count < n:
        ...
        shortest_job = min(available_procs, key=lambda x: x['burst'])
        ...
        shortest_job['done'] = True
        completed_count += 1
        
        log.append(f"[{start_t:.3f}] -- {shortest_job['id']} -- [{end_t:.3f}]")
        
        tat = end_t - shortest_job['arrival']
        wt = tat - shortest_job['burst']
...
for f_name in files:
    print(f"Processing {f_name} for SJF...")
    data = read_file(f_name)
    if not data: continue

    wt, tat, completed, cs, total_time, logs = run_sjf_np(data)
    ...
    out_name = f"SJF_NP_Result_{f_name}.txt"
